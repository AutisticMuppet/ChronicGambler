...
def play_game(li: lichess.Lichess,
    ...
    with li.get_game_stream(game_id) as response:
        ...
        with engine_wrapper.create_engine(config, game) as engine:
            skill_level_value = engine.options.get("UCI_Elo") or engine.options.get("Skill Level") or game.skill_level
            if isinstance(skill_level_value, list):
                skill_level_value = skill_level_value[0] if skill_level_value else None
            game.skill_level = skill_level_value
            ...
            keyword_map: defaultdict[str, str] = defaultdict(
                ...
                skill_level=str(game.skill_level if game.skill_level is not None else "unknown"),
            )
            hello = get_greeting("hello", config.greeting, keyword_map)
            goodbye = get_result_greeting("goodbye", config.greeting, keyword_map, game)
            hello_spectators = get_greeting("hello_spectators", config.greeting, keyword_map)
            goodbye_spectators = get_result_greeting("goodbye_spectators", config.greeting, keyword_map, game)
...
def get_greeting(greeting: str, greeting_cfg: Configuration, keyword_map: defaultdict[str, str]) -> str:
    ...
    if not isinstance(greeting_text, str) or not greeting_text:
        return ""
