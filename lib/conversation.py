...
class Conversation:
    ...
    def react(self, line: ChatLine) -> None:
        ...
        text = line.text if isinstance(line.text, str) else (str(line.text) if line.text is not None else "")
        logger.info(f"*** {self.game.url()} [{line.room}] {line.username}: {text}")
        if text.startswith(self.command_prefix):
            self.command(line, text[1:].lower())
    ...
    def command(self, line: ChatLine, cmd: str) -> None:
        ...
        elif cmd == "spinthewheel": #This allows ChronicGambler to spoil its skill level
            skill_level = self.game.skill_level if self.game.skill_level is not None else "unknown"
            self.send_reply(line, f"I am currently playing at an ELO of: ~ {skill_level}")
        elif cmd == "name":
    ...
    def send_message(self, room: str, message: str) -> None:
        ...
        if message:
            payload = {"room": room, "username": getattr(self.game, "username", ""), "text": message}
            self.send_reply(ChatLine(payload), message)
