...
class Game:
    ...
    def __init__(self, game_info: GameEventType, username: str, base_url: str,
        ...
        try:
            _skill = config.engine.uci_options.lookup("UCI_Elo")
        except Exception:
            _skill = None
        if not _skill:
            try:
                _skill = config.engine.uci_options.lookup("Skill Level")
            except Exception:
                _skill = None
        if isinstance(_skill, list):
            self.skill_level = _skill[0] if _skill else None
        else:
            self.skill_level = _skill
